import 'package:flutter/cupertino.dart';
import 'package:flutter/material.dart';
import 'package:food_delivery_app/utils/screen_colors.dart';

class InputPassword extends StatefulWidget {

  final String inputName;
  final String inputTitle;
  final String? image;
  final TextEditingController controllerName;
  final VoidCallback? onPressed;

  const InputPassword({super.key, required this.inputName, required this.inputTitle,  this.image, required this.controllerName,  this.onPressed});


  @override
  State<InputPassword> createState() => _InputBoxState();
}

class _InputBoxState extends State<InputPassword> {
  //obsecurePass is for using password visibility on and off...
  ValueNotifier obSecurePass=ValueNotifier(true,);

  @override
  Widget build(BuildContext context) {
    return Column(
      mainAxisAlignment: MainAxisAlignment.start,
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        Text(widget.inputTitle,style: TextStyle(
          fontFamily: 'FontSen',
          fontWeight: FontWeight.w400,
          color: SColors.textColorBlack,
          fontSize: 18,
        ),),
        SizedBox(height: MediaQuery.of(context).size.height * 0.01),
        // password box..
        ValueListenableBuilder(
            valueListenable: obSecurePass,
            builder: (context,value,child){
              return TextFormField(
                obscureText: obSecurePass.value,

                decoration: InputDecoration(
                  filled: true,
                  fillColor: SColors.inputBoxColorLightGreen,
                  focusedBorder: OutlineInputBorder(
                    borderSide: BorderSide(color: SColors.inputBoxColorLightGreen),
                    borderRadius: BorderRadius.circular(10),
                  ),
                  suffixIcon: GestureDetector(
                    onTap: (){
                      obSecurePass.value=!obSecurePass.value;
                    },
                    child: obSecurePass.value
                        ?Icon(Icons.visibility_off)
                        :Icon(Icons.visibility),
                  ),
                  enabledBorder: UnderlineInputBorder(
                    borderSide: BorderSide(color:SColors.inputBoxColorLightGreen ),
                    borderRadius: BorderRadius.circular(10),
                  ),
                ),
                style: TextStyle(fontSize: 33,
                  fontFamily: 'FontSen',
                  fontWeight: FontWeight.w400,
                  color: SColors.textColorBlack,
                ),
              );
            }
        ),
      ],
    );
  }
}
